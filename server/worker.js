...
const worker = new Worker('video-processing', async (job) => {
  ...
  try {
    ...
    if (!updatedLesson) {
        console.error(`❌ Error: Lesson ${lessonId} not found in database during status update.`);
    } else {
        console.log(`✅ Database updated. HLS Path: ${updatedLesson.video.hlsPath}`);
    }
    ...
    console.log(`⚠️ Lesson ${lessonId} status set to 'failed'. Retrying via BullMQ...`);
