...
export const presignUpload = async (req, res) => {
  try {
    ...
    if (!lessonId || !fileType)
      return res
        .status(400)
        .json({ message: "lessonId and fileType required" });
    ...
    if (lesson.course.instructor.toString() !== user.id.toString()) {
      return res
        .status(403)
        .json({ message: "Not allowed to upload for this lesson" });
    }
...
export const getPlaybackUrl = async (req, res) => {
  try {
    ...
    // Access control
    const isInstructor =
      lesson.course.instructor.toString() === req.user.id.toString();
...
export const completeUpload = async (req, res) => {
  try {
    ...
    if (!lessonId || !fileKey)
      return res.status(400).json({ message: "lessonId and fileKey required" });
    ...
    if (lesson.course.instructor.toString() !== user.id.toString()) {
      return res
        .status(403)
        .json({ message: "Not allowed to complete upload for this lesson" });
    }
